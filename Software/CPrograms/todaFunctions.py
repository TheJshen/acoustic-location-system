...
def frikkFilterDesigner(centerFreq, pBandwidth, filterOrder, Fs):
	...
	#centerFreq and with a passband with of pBandwidth(or pBandwidth/2 to each
...
def filterAndSmooth( X, b, smoothingCoeff ):
	...
	np.savetxt('y.txt', Y, fmt='%lf', delimiter=', ')   # X is an array
	YMA = movingAverage(np.absolute(Y), smoothingCoeff)
	#YMA = movingAverage(Y, smoothingCoeff)
	
	np.savetxt('yma.txt', YMA, fmt='%10.0lf', delimiter=', ')   # X is an array
	return YMA
