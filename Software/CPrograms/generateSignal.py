...
Fs = 250000  #Sampling frequency, Should be no problem to change this
...
#x1 = 1365 * (np.sin(2*pi*f1*t) + 1);
#x2 = 1365 * (np.sin(2*pi*f2*t) + 1);
#x3 = 1365 * (np.sin(2*pi*f3*t) + 1);
...
x1Rec = np.concatenate((x1Delay, x1Sig), axis = 0);
x2Rec = np.concatenate((x2Delay, x2Sig), axis = 0);
x3Rec = np.concatenate((x3Delay, x3Sig), axis = 0);

#Save each received signal to file
np.savetxt('x1Rec.txt', x1Rec, fmt='%.0f', delimiter=', ')   # X is an array
np.savetxt('x2Rec.txt', x2Rec, fmt='%.0f', delimiter=', ')   # X is an array
np.savetxt('x3Rec.txt', x3Rec, fmt='%.0f', delimiter=', ')   # X is an array
...
pbw = 500; # Passband width in hertz
fOrder = 64; # Filter order number

b1 = frikkFilterDesigner( f1, pbw, fOrder, Fs );
b2 = frikkFilterDesigner( f2, pbw, fOrder, Fs );
b3 = frikkFilterDesigner( f3, pbw, fOrder, Fs );

np.savetxt('coeffsA.txt', b1, fmt='%f', delimiter=', ') 
np.savetxt('coeffsB.txt', b2, fmt='%f', delimiter=', ') 
np.savetxt('coeffsC.txt', b3, fmt='%f', delimiter=', ') 
...
#plt.figure(1)
#plt.plot(extracted1, '--r', x1Rec, '--b')
#plt.subplot(312)
#plt.plot(x1Rec)
#plt.title('X1 filtered from recieved signal')

#plt.figure(2)
#plt.plot(extracted2, '--r', x2Rec, '--b')

plt.plot(extracted1);
plt.title('X1 filtered from recieved signal')

#plt.plot(x1Rec);
#plt.title('X1 as sent');

#plt.plot(extracted2);
#plt.title('X2 filtered from recieved signal')

#plt.plot(x2Rec);
#plt.title('X2 as sent');

#plt.plot(extracted3);
#plt.title('X3 filtered from recieved signal')

#plt.plot(x3Rec);
#plt.title('X3 as sent');

#plt.plot(0.02)

#plt.figure(3)
#plt.plot(extracted3, '--r', x3Rec, '--b')
#plt.title('X3 filtered from recieved signal')
...
detectionThreshold = .03; # A threshold for how strong a signal needs to be on order to be detected as a recieved pulse.
