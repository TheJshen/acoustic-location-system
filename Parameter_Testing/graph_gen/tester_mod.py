...
plt.figure()
plt.plot(x1Sig[0:x1Length],color='red')
plt.title('X1 Raw signal without any processing')
plt.xlabel('Time')
plt.ylabel('Y = sin(2 * t * pi * frequency ) ')

plt.figure()
plt.xlabel('Time')
plt.ylabel('Y = sin(2 * t * pi * frequency ) ')
plt.plot(x2Sig[1:x2Length],color='blue')
plt.title('X2 Raw signal without any processing')
plt.figure()
plt.xlabel('Time')
plt.ylabel('Y = sin(2 * t * pi * frequency) ')
plt.plot(x3Sig[1:x3Length],color='green')
plt.title('X3 Raw signal without any processing')
#plt.show()

#Creating Recieved Signal
T =  15;                                         #Temp in C
S = 5;                                          #Salinity in parts per thousand
D = 10;                                            #Depth (0 - 100) in meters
...
plt.figure()
plt.plot(x1Rec[0:recSigLength],color='red')
plt.title("X1: Rectified Signal")
plt.xlabel('Time rectified')
plt.ylabel('Y = sin')
plt.figure()
plt.plot(x2Rec[0:recSigLength],color='blue')
plt.xlabel('Time rectified ')
plt.ylabel('Y = sin')
plt.title("X2: Rectified Signal")
plt.figure()
plt.plot(x3Rec[0:recSigLength], color='green')
plt.xlabel('Time rectified')
plt.ylabel('Y = sin')
plt.title("X3: Rectified Signal")
#plt.legend(loc='best')
#plt.title("Same three signals, now corrected to be all the same size")
#print ("Done")
...
plt.figure()
plt.plot(recSignal,color='black')
plt.xlabel('Time')
plt.ylabel('Y = sin ')
...
fOrder = 2048; # Filter order number
...
plt.figure()
plt.plot(extracted1,label='X1 filtered from recieved signal',color='red')
plt.xlabel('Time')
plt.ylabel('Filtered by FIR ')
plt.title('Three signals that would be recovered after filtering')
#plt.figure()
plt.plot(extracted2,label='X2 filtered from recieved signal',color='blue')
#plt.title('X2 filtered from recieved signal')
#plt.figure()
plt.plot(extracted3,label='X3 filtered from recieved signal',color='green')
